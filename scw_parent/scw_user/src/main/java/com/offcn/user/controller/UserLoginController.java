...
public class UserLoginController {
    ...
    public AppResponse<Object> sendCode(String phoneNo) throws Exception{
        //1、生成验证码保存到服务器，准备用户提交上来进行对比
        String code = UUID.randomUUID().toString().substring(0, 4);
        //2、保存验证码和手机号的对应关系,设置验证码过期时间
        redisTemplate.opsForValue().set(phoneNo, code, 120, TimeUnit.HOURS);
        //3、短信发送构造参数
    ...
    public AppResponse<Object> regist(UserRegistVo registVo) {
        ...
        if (!StringUtils.isEmpty(code)) {//使用字符串工具类判断是否为空
            //redis中有验证码  忽略大小写的比对
            boolean b = code.equalsIgnoreCase(registVo.getCode());
            if (b) {
                //2、将vo转业务能用的数据对象
                TMember member = new TMember();
                //将registVo字段的数据拷给menber中
